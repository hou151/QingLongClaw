...
if [[ "${HOST_ARCH}" == "x86_64" || "${HOST_ARCH}" == "amd64" ]]; then
  ...
  if ! command -v x86_64-linux-gnu-g++ >/dev/null 2>&1 && ! command -v x86_64-linux-gnu-g++-11 >/dev/null 2>&1; then
    echo "error: x86_64 cross compiler not found: x86_64-linux-gnu-g++" >&2
    echo "hint: install x86_64-linux-gnu-g++ or run this script on an x86_64 host." >&2
    exit 1
  fi
  cmake -S "${ROOT_DIR}" \
    -B "${BUILD_DIR}" \
    -DCMAKE_BUILD_TYPE=Release \
    -DCMAKE_TOOLCHAIN_FILE="${ROOT_DIR}/toolchains/linux-x86_64-gcc.cmake"
fi
...
if [[ "${HOST_ARCH}" == "x86_64" || "${HOST_ARCH}" == "amd64" ]]; then
  ...
  if command -v x86_64-linux-gnu-g++-11 >/dev/null 2>&1; then
    SELECTED_CXX="$(command -v x86_64-linux-gnu-g++-11)"
  elif command -v x86_64-linux-gnu-g++ >/dev/null 2>&1; then
    SELECTED_CXX="$(command -v x86_64-linux-gnu-g++)"
  fi
fi
