...
if [[ "${HOST_ARCH}" == "aarch64" ]]; then
  ...
  if ! command -v aarch64-linux-gnu-g++ >/dev/null 2>&1 && ! command -v aarch64-linux-gnu-g++-11 >/dev/null 2>&1; then
    echo "error: aarch64 cross compiler not found: aarch64-linux-gnu-g++" >&2
    echo "hint: install aarch64-linux-gnu-g++ or run this script on an aarch64 host." >&2
    exit 1
  fi
  cmake -S "${ROOT_DIR}" \
    -B "${BUILD_DIR}" \
    -DCMAKE_BUILD_TYPE=Release \
    -DCMAKE_TOOLCHAIN_FILE="${ROOT_DIR}/toolchains/linux-aarch64-gcc.cmake"
fi
...
if [[ "${HOST_ARCH}" == "aarch64" ]]; then
  ...
  if command -v aarch64-linux-gnu-g++-11 >/dev/null 2>&1; then
    SELECTED_CXX="$(command -v aarch64-linux-gnu-g++-11)"
  elif command -v aarch64-linux-gnu-g++ >/dev/null 2>&1; then
    SELECTED_CXX="$(command -v aarch64-linux-gnu-g++)"
  fi
fi
